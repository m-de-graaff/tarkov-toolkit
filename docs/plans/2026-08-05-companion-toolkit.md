...
- [x] `apps/watcher/package.json` (name `@raidplanner/watcher`, dep `ws`, dep `@raidplanner/live`, script `start`: `node --experimental-strip-types src/index.ts`; root script `watcher`), `src/index.ts`: resolve screenshots dir (`%USERPROFILE%\Documents\Escape from Tarkov\Screenshots`, overridable via argv/env), `fs.watch` + 2s poll fallback listing, broadcast protocol per constraints, console lines a non-dev can read ("Watching …", "Position sent: x,y,z").
- [x] Verify by running it against the real folder: starts, logs the existing screenshot broadcast; `node -e` WS client receives hello + fix. Commit.

### Task 2: web companion transport
- [x] `useLiveWatcher`: `companion` state ('absent' | 'connected'); on mount open `ws://127.0.0.1:17520`, retry 5s on close/error (cleanup on unmount); on fix message `setLiveFix`; FSA path untouched; when companion connected, FSA controls hidden. `LivePanel`: "Companion app is watching your screenshots" + dot when connected; otherwise existing button plus a muted "or run the companion app (any browser)" hint linking README anchor.
- [x] Test setup WebSocket stub; new hook test: simulate stubbed socket firing hello+fix → store updated (drive the stub manually).
- [x] Suite + build green, commit.

### Task 3: toolkit shell + scrollbars
- [x] TopNav wordmark → "Tarkov Toolkit", tab label "Raid Planner"; `index.html` title; README intro reframed (toolkit; current tool: raid planner + progress; roadmap: barter calculator, flea prices); scrollbar CSS.
- [x] Suite + build green, commit.
